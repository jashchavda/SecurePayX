...
public class NotificationService {
    ...
    public void consumePaymentFailed(@Payload Map<String, Object> payload) {
        try {
            ...
            String reason = (String) payload.getOrDefault("reason", "Transaction declined");

            if (accountNumber == null || accountNumber.isBlank()) {
                log.warn("Received payment.failed event with missing account number");
                return;
            }

            String alertMessage = String.format(
                    "Your payment of %s could not be processed. Reason: %s. Please try again or contact support.",
                    amount, reason
            );

            sendAlert(accountNumber, "PAYMENT FAILED", alertMessage);
